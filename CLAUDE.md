...
- `migrations/` Alembic. `seed_dev.py` demo content (uses the importer). `load_sentences.py` CLI that loads a sentence artifact. `audit_content.py` CLI that reports items whose meanings look unanswerable (prose glosses, alphabet-letter boilerplate). `tests/` pytest suite, `conftest.py` fixtures.

Content quality: raw Wiktionary glosses are curated by `pipeline/curate.py` (short typeable answers into `translation_primary`/`translations`, the original prose into `notes`, alphabet letters dropped). The importer rejects `translation_primary`/`translations` entries over 80 chars. `pipeline/cleanup_letter_items.py` removes pre-curation letter items from a live database.
