...
- `routers/` HTTP endpoints, one module per area: auth, lessons, reviews (`/reviews/forecast`), dashboard, study, billing, settings, items, push, stats, account (`/account/export`, `/account/delete`), internal (cron-triggered `/internal/push/run` and `/internal/email/digest` behind `X-Internal-Token`).
